...
for (n in 1:nrow(param)){
  ...
  untar(tarfile=paste(args[1],"_",param["job_id"][n,],".tar.gz",sep=""),
        files = c(paste(args[1],"_",param["job_id"][n,],"/GeneticsOutput/settings",param$generations[n],
                      ...
                      "_",param$MutationRate[n],"_",param$MainCarryingCapacity[n],
                      "_GeneSamples_",seq(1,10,1),".arp" ,sep=""),
  ...
  genotype = genotype[which(group$V1 != "MR"),]
  group = group[which(group$V1 != "MR"),]
  group = as.numeric(as.vector(group[,2]))


  spect = apply(genotype, MARGIN = 2, FUN = function(x) {maf(as.numeric(as.factor(x)) - 1)}   ) #create SFS
  ...
  z = sapply(1:111, FUN = function(x) sum(ind1 == x) )

  stat.sim <- c((histo.bin(spect)/sum(histo.bin(spect))), z.groupm(z)/sum(z.groupm(z)) ) # compute summary stats
  names(stat.sim) <- c(paste("SFS",seq(1,7,1), sep=""),paste("RareVariants",seq(1,13,1), sep=""))
  stat.temp <- as.data.frame(c(param[n,],stat.sim))
  spect.temp <- as.data.frame(c(param[n,],spect,as.data.frame(arrival)))

  sum.stat <- rbind(sum.stat, stat.temp )
  spect.stat <- rbind(spect.stat,spect.temp)
  cond <- c(rep("SFS",7),rep("RareVariants",13))

  print("saving computed statistics to file")
}
print("saving computed statistics to file")
write.table(file = paste(args[2],".moinsMR_sum.stat.txt",sep=""), sum.stat, row.names = F, quote=F)
write.table(file = paste(args[2],".moinsMR_sum.spect.stat.txt",sep=""), spect.stat, row.names = F, quote=F)
